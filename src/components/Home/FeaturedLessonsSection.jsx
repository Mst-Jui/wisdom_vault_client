...
const FeaturedLessonsSection = () => {
  ...
  // featured lessons load
  useEffect(() => {
    const loadFeatured = async () => {
      try {
        setLoading(true);
        const data = await serverFetch(`/api/lessons/featured?limit=6`);
        setLessons(data?.data || []);
      } catch (error) {
        console.error("Featured lessons load error:", error);
      } finally {
        setLoading(false);
      }
    };
...
export default FeaturedLessonsSection;